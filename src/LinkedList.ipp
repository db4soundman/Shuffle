...
template <class T>
typename LinkedList<T>::Node* LinkedList<T>::find(unsigned int i){
   if (i==numItems) {
      return dummyNode;
    }
    if(i>numItems)
    ...
    if(i!=numItems-1){
        ...
        for (unsigned int location = 0; location<i; location++) {
...
template <class T>
void LinkedList<T>::set(unsigned int i, T x){
    Node* nodeToChange = find(i);
    nodeToChange->data = x;
}

template <class T>
void LinkedList<T>::add(unsigned int i, T x){
...
template <class T>
void LinkedList<T>::splice(unsigned int i, unsigned int len, LinkedList<T>& target, unsigned int t){
